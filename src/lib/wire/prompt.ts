...
function markedPaperBlock(): string[] {
  return [
    ...
    "",
    "  1. File the whole paper as an entries[] row in THIS SAME payload",
    "     — a topicMarks row is never evidence for a result the tape",
    "     cannot show, and the dual FK below only resolves WITHIN one",
    "     payload, never against the book. If the paper is already on",
    "     the tape, re-emit its entries[] row here too (its existing,",
    "     deterministic id — this is idempotent, not a duplicate).",
...
function interviewBlock(opts: WireOpts): string[] {
  return [
    ...
    "  3. Anything coming up? When, what type, worth how much, and what",
    "     time of day it starts if they know it. (⇒ hour.)",
    ...
    "     – 5 (best). (⇒ belief.) Their attendance rate, if they know",
    "     it. (⇒ attendancePct.) No document states either.",
    "",
